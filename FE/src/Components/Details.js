...
class Details extends Component {
    ...
    componentDidMount = async () => {
        ...
        const result = await axios({
            method: "GET",
            url: `http://localhost:9876/restaurants/${restaurant_id}`,
    ...
    handleMenuModal = async () => {
        ...
        const result = await axios({
            method: 'GET',
            url: `http://localhost:9876/getmenu/${restaurant_id}`,
            headers: { 'Content-Type': 'application/json' }
        });
    ...
    makePayment = (token) => {
        ...
        return fetch(`http://localhost:9876/payment`, {
            method: "POST",
            headers,
    ...
    render() {
        ...
        return (
            ...
            <div >
                {/* <img className='details' src='./assets/details.jpg'/> */}
                ...
                <div className="container" style={{}}>
                    <img src={restaurant.thumb} className="detail-img" alt="restaurant-image"  />
                    ...
                    <div id="content1" className="contentDetails" style={this.state.content_value === 1 ? { display: 'block' } : { display: 'none' }}>
                        {/* <h1 className="restaurant-name2">About this place</h1> */}
                        <div className="cuisine-detail">Cuisine : </div>
                        <div className="cuisine-item">{restaurant.cuisine_id ? restaurant.cuisine_id.map(item => ` ${ item.name} `) : null}</div><br/>
                        <div className="cuisine-detail">Average Cost : </div>
                        <div className="cost"> ₹ {restaurant.cost} for two people (approx)</div>
                        <button className="order-button" onClick={this.handleMenuModal}>Place Online Order</button>

                    </div>
                    <div id="content2" className="contentDetails" style={this.state.content_value === 2 ? { display: 'block' } : { display: 'none' }}>
                        {/* <div className="phone-number">Contact :</div> */}
                        <h1 className="restaurant-name2">{restaurant.name}</h1>
                        <div className="number" > + { restaurant.contact_number}</div>
                    ...
                    {/* This is Image-gallery maodal */}
                     <Modal isOpen={galleryModalIsOpen} style={customStyles}>
                        <h2 style={{display:'inline-block',marginTop:'-5px',color:'white'}}>{restaurant.name} Place</h2>
                        <button className="carousel-closebutton" style={{ float: 'right',background:'white',border:'none',borderRadius:'5px',marginBottom:'10px' }} onClick={this.closeModal}> <Icon.BackspaceFill/> </button>
                        <div>
                            <Carousel showThumbs={false}>
                                <div>
                                    <img src="Assets/breakfast.jpeg" className="carousel-image" />
                                </div>
                                <div>
                                    <img src="Assets/lunch.jpeg" className="carousel-image" />
                                </div>
                                <div>
                                    <img src="Assets/dinner.jpeg" className="carousel-image" />
                                </div>
                                <div>
                                    <img src="Assets/snacks.jpeg" className="carousel-image" />
                                </div>
                                <div>
                                    <img src="Assets/drinks.jpeg" className="carousel-image" />
                                </div>
                                <div>
                                    <img src="Assets/nightlife.jpeg" className="carousel-image" />
                                </div>
                            </Carousel>
                        </div>
                    </Modal> 
                    ...
                    <Modal
                        ...
                        style={menuStyles}
                    >
                        <button className="carousel-button" onClick={this.closeMenuModal}> <Icon.BackspaceFill/> </button>
                        <div className="menu-modal">
                            ...
                            <div className="container">
                                ...
                                {menuItems?.map((item, index) => {
                                    return <div className=" menu-container">
                                        ...
                                        <div className='itemDiv'>
                                            <img src={item.image_url} alt="" height="50" width="50" className='menuImage' style={{ borderRadius: '10px'}} />
                                            {item.qty === 0 ? <div className="add-button" onClick={() => this.handleAdd(index, "add")}>Add</div> :
                                                <div style={{ textAlign: 'center',marginBottom:'20px' }}>
                                                    <span style={{ fontSize: '15px', color: 'white', fontWeight: '600', marginRight: '5px', cursor: 'pointer' }} onClick={() => this.handleAdd(index, "subtract")}><Icon.DashCircleFill/></span>
                                                    <span style={{ fontSize: '15px', color: 'white', fontWeight: '600', marginRight: '5px' }}>{item.qty}</span>
                                                    <span style={{ fontSize: '15px', color: 'white', fontWeight: '600', marginRight: '5px', cursor: 'pointer' }} onClick={() => this.handleAdd(index, "add")}><Icon.PlusSquareFill/></span>
                                                </div>}
                                        </div>
                            ...
                            <div className="Rectangle-3352">
                                ...
                                <span className="Rectangle-3353" onClick={this.handlePayButton}>Continue <Icon.CaretLeftFill/></span> 
                            </div>
                        </div>
                    ...
                    <Modal
                        ...
                        <div className='clearCartDiv'>
                            <button className="carousel-button" onClick={this.closehandleAlert}><Icon.BackspaceFill/></button>
                            <div className='cart'>Clear your cart?</div>
                            <hr />
                            <div className='cartContent'>Your cart has existing items from <strong>{this.state.oldCartRestaurant.name}</strong>. <br/> Do you want to clear it and add items from <strong>{this.state.restaurant.name}</strong>?</div>
                            <div style={{ margin: '10px 0' }}>
                                <div style={{ padding: '10px' }}>
                                    <div style={{ fontSize: '24px', fontWeight: 'bolder', marginTop:'10px', color: 'white' }}>{this.state.oldCartRestaurant.name}</div>
                                ...
                                {this.state.cartMenuitems.map((item) => {
                                    return <div className='cartitem'>
                                        ...
                                        <span className='menuCon'style={{ fontSize: '14px'}}>{item.qty} &#215; {item.name}</span>
                                        <span className='menuPrice' >₹{item.price * item.qty}</span>
                                        <hr style={{ margin: '0' }} />
                                    </div>
                                })}
                            </div>
                            <div style={{ float: 'right' }}>
                                <button className='cartBtn' onClick={this.clearCart}>Clear cart</button>
                                <button className='cartBtn' onClick={this.goToCart}>Proceed with these items</button>
                            </div>
...
//         const result = await axios({
//             method: "GET",
//             url: `http://localhost:9876/restuarent/${restaurant_id}`,
//             headers: { 'Content-Type': 'application/json' }
//         })
//         this.setState({ restaurant: result.data, restaurant_id });
...
//     // through the handle alert we can navigate to cart through this function.
//     goToCart = () => {
//         this.props.history.push(`/details?restaurent=${this.state.oldCartRestaurant._id}&open=${true}`);
//         window.location.reload();
//     }
//     // This is method close our payment form
//     closePaymentForm = () => {
...
//                 {/* This is whole container of details page */}
//                 <div className="container" style={{}}>
//                     <img src={restaurant.thumb} className="detail-img" alt="restaurant-image" width="1073px" height="352px" style={{ marginTop: "50px" }} />
//                     <button className="gallery-button" onClick={this.handleGalleryModal}>Click to see Image Gallery</button>
//                     <div className="restaurant-name" style={{fontSize:"40px"}}>{restaurant.name}</div>
//                     <button className="order-button" onClick={this.handleMenuModal}>Place Online Order</button>
...
//                         <div className="restaurant-name2" style={{fontSize:"40px", color:"rgb(167, 4, 4)"}}>About this place</div>
//                         <div className="cuisine-detail" style={{fontSize:"30px", color:" rgb(255, 42, 42)"}}>Cuisine</div>
//                         <div className="address" style={{ marginBottom: '30px' }}>{restaurant.cuisine_id ? restaurant.cuisine_id.map(item => `${item.name} `) : null}</div>
//                         <div className="cuisine-detail" style={{fontSize:"35px", color:" rgb(255, 42, 42)"}}>Average Cost</div>
...
//                         <div className="phone-number" style={{ marginTop: '10px', fontSize:"35px", color:" rgb(255, 42, 42)" }}>Phone Number</div>
//                         <div className="number" style={{ marginBottom: '30px' }}>{restaurant.contact_number}</div>
//                         <div className="restaurant-name2" style={{fontSize:"35px", color:" rgb(255, 42, 42)"}}>{restaurant.name}</div>
...
// //         // url: `http://localhost:9876/restaurantss/${restaurants}`,
// //         headers: { 'Content-Type': 'appication/json' }
// //     })
// //         .then(res => {
// //             this.setState({ restaurants: res.data.restaurants, restaurantsId:restaurants })
...
// //                 <img className="imgD" src="https://img.freepik.com/free-photo/exploding-burger-with-vegetables-melted-cheese-black-background-generative-ai_157027-1734.jpg" alt="Error" />
// //                 <button className='order'>Place Online Order</button>
// //             </div>
// //             <h1 className='headingD'>{restaurants.name}</h1>


// //             <div className='tab'>
// //             <ul className='d flex'>
// //                     {/* <li className='flex-fill' onClick={()=>Updatetoggle(1)}>Overview</li> */}
// //                     {/* <li className='flex-fill' onClick={()=>Updatetoggle(2)}>Contact</li> */}

// //                     <li className='flex-fill'>Overview</li>
// //                     <li className='flex-fill'>Contact</li>
// //                 </ul>
// //             </div>
// //             <p className="uline"></p>
...
// //             <div >
// //             {/* <div className={toggle === 1 ? "showContent" : "contents"}> */}

// //                 <h3>About this Places</h3>
// //                 <h5>Cuisine</h5>
// //                 <p>Bakery, Fast-food</p>
// //                 <p>Average Cost</p>
// //                 <p>#8377 {restaurants.min_price} for two peoples(approx)</p>
// //             </div>

// //             {/* <div className={toggle === 2 ? "showContent" : "contents"}> */}
// //                 <div>            
// //                 <h3>Phone Number</h3>
